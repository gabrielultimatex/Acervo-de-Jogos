...
- **Jogatina** (ícone de controle, ao lado da Lista de Desejos): ferramentas úteis durante a partida em si, sem depender de um jogo específico:
  - **Cronômetro**: regressivo (com aviso sonoro nos 5 segundos finais e alarme no zero, mais botões de ajuste de tempo) ou progressivo (conta pra cima), com iniciar/pausar/reiniciar. Continua rodando em segundo plano mesmo navegando pra outra tela do site.
  - **Placar**: escolhe quem está na mesa e vai somando/subtraindo pontos por pessoa (+1, +5, -1, -5). Um checkbox libera pontuação negativa (pra jogos onde se perde pontos), e dá pra zerar só uma pessoa ou todo mundo de uma vez.
  ...
  - **Roleta**: sorteia alguém entre quem está na mesa — roda colorida girando com um ponteiro fixo por fora (não gira junto), e confete ao parar no vencedor.
  - **Girar a Garrafa**: não depende de selecionar ninguém — é só uma garrafa (desenho mais detalhado, com gargalo, rótulo e tampa) girando de verdade no centro da mesa; a pessoa escolhida é quem estiver sentada na direção que o bico apontar quando parar.
